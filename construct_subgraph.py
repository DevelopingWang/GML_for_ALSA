...
class ConstructSubgraph:
    ...
    def construct_subgraph_for_mixture(self, k_id_list,evidences):
    ...
    def construct_subgraph_for_unaryPara(self,var_id,evidences):
        '''在选出topk个隐变量之后建立子图
        输入：一个隐变量的id
        ...
        if self.balance:
            ...
            for varid in evidence_set:
                if self.variables[varid]['label'] == 1:
                    label1_var.add(varid)
                elif self.variables[varid]['label'] == 0:
    ...
    #待修改
    def construct_subgraph_for_ER2(self, var_id,evidences):
        '''第二种建图方式
        输入：一个隐变量的id
        输出：按照numbskull的要求因子图,返回weight, variable, factor, fmap, domain_mask, edges
        '''
