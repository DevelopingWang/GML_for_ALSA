...
class GML:
    ...
    def __init__(self,variables, features, evidential_support_method, approximate_probability_method,evidence_select_method,top_m=2000, top_k=10, update_proportion=0.01,
        ...
        self.construct_subgraph = ConstructSubgraph(variables, features)
        logging.basicConfig(
    ...
    def inference_subgraph(self, var_id):
        ...
        ns = numbskull.NumbSkull(n_inference_epoch=10,
                                 ...
                                 reg_param=0.01)
        if self.evidence_select_method == 'interval':
            weight, variable, factor, fmap, domain_mask, edges_num, var_map = construct_subgraph.construct_subgraph_for_ER(var_id)
        else if self.evidence_select_method == 'ralation':
            weight, variable, factor, fmap, domain_mask, edges_num, var_map = construct_subgraph.construct_subgraph_for_ALSA(var_id)
